...
- `docs/mobile-redesign` (local == origin == `253e9ff`) still carries a **duplicate billing cherry-pick** on top of the spec `ed418a6` (spec/plan content unchanged since `ed418a6`, verified). Harmless for a docs-only branch; optional cleanup back to `ed418a6`.
...
- **CORRECT FIX — applied by Viktor in `f48d122` (test-only, ZERO production lines):** supply `billingAddress: nil` explicitly at the two `OrdersDeliveryMethodTests` call sites (`:196`, `:221`).
