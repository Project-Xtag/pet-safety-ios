...
- **G12 — iOS cold-launch universal link may be dropped during the splash** *(new 2026-07-12; latent, exacerbated by C0-iOS).* `.onOpenURL` is attached only inside `ContentView` (`PetSafetyApp.swift:180` on the `else` branch + `ContentView.swift:102`), which does **not** exist while `showSplash == true`. So a cold-launch `senra.pet/t/{id}` arriving during the splash has **no handler in the hierarchy**; whether SwiftUI replays it when `ContentView` appears or drops it is version-dependent and **must be QA'd, not inferred**. C0-iOS widened the window `0.8s → 2.0s` (~2.5×), increasing exposure. **Fix belongs in C1** (which restructures this gate): capture the URL **above** the gate into `@State` and consume it when `ContentView` appears — mirroring the Android `pendingQrCode` capture. **QA (both platforms):** cold start from a tag URL, app installed → land on the pet page, not landing/home.
