...
- **G11 — Pending scan is stranded when the gate drops to a logged-out user** *(new 2026-07-12; pre-existing, surfaced by C0's true gate).* Android captures a cold-start tag QR above the splash gate (`pendingQrCode → savedQrCode`), but if the user is **unauthenticated** when the gate drops, `screenKey → "auth"` and `savedQrCode` is never consumed — the scan silently does nothing. Pre-existing (not a C0 regression), but **Phase 1 makes the landing the logged-out default and scan a public surface**, so **C2/C3 must decide what a pending QR does on the landing** (route it to the public scan/pet path, don't drop it). Logged now while it's visible.
