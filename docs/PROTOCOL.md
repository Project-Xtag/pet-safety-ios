...
| **zsh is not bash.** `#` is **not a comment** interactively, and `$VAR:P` is a **parameter modifier** — `$T:PetSafety/...` silently rewrites a hash into a path. **`git show "$REF:path"` is the same trap**, and it fired three times on 2026-08-02 *in the session that promoted this row.* | **One command per line. No inline comments.** Inline the value, don't interpolate before a `:`. **BRACE IT: `"${REF}:path"`.** For `git show`, use the `gshow` helper below rather than remembering — knowledge was never the gap here. |
...
| **A hash of `e3b0c44298fc`** — the artifact is EMPTY (sha256 of zero bytes), usually an unfetched ref or a failed command whose `>` still created the file. **The failure mode is not "wrong hash" — it is empty file, plausible hash, NO ERROR**, which is why it survives a careful reader. | Rule 4. Print `wc -c` beside every hash; learn the prefix. **Better: make it unskippable with `gshow` below**, which returns non-zero and prints *nothing* on a bad ref instead of a plausible value. |
...
| **A grep returning 0 with no control** — case (`rateLimiter` vs `createRateLimiter`), wrong field names (`line1` vs `street1`), or `grep -c` counting lines not occurrences in minified output. | Rule 4. Run a control that must hit, in the same command. **The control must be a phrase you KNOW is present AND subject to the same suspected defect** — same file, same case sensitivity, same encoding. A generic control (`grep -c 'the'`) proves the file is being read; it does **not** test the pattern, and it passes while the real query is broken. **The signal you are looking for is TWO zeros where one was impossible:** that is what localises the fault to the pattern rather than the tree. *Precedent 2026-08-02: a `§D.S` pattern read 0 against `§D's` — a case mismatch — and only a control sharing the same case sensitivity exposed it.* |
...
| **🔴 AN INSTRUMENT THAT DEFEATS ITSELF — the fault is in the technique, so retrying it harder never works.** To measure a timeout you must make the network hang. But **breaking the network is exactly what modern OSes are built to route around.** iOS *Wi-Fi Assist* fails over to cellular the instant Wi-Fi stops answering, so the blackhole ends the hang it was created to cause; Android reads the system proxy at **process start**, so a blackhole applied to a running app is ignored, and applying it *before* launch kills the login needed to reach the surface under test. Both were measured on 2026-08-08, and neither is a misconfiguration — the technique is self-cancelling on both platforms. **The tell is a green-looking result with no evidence: the app "worked", nothing hung, and the instrument recorded nothing.** Three attempts went to plumbing before the mechanism was named. | **Prove the instrument on the target BEFORE spending a fixture on it** — a control that shows the phone's traffic actually reaching your observer, not just that the observer is reachable from your own machine. (Both of the first run's `ACCEPT`s came from the laptop's own self-tests; zero came from the phone, and that was the whole finding.) Then prefer an instrument that leaves the link **healthy** so failover has no trigger: intercept **resolution**, not the route — answer one hostname with your own address and accept-without-replying, so DNS is instant, TCP connects instantly, and only the one call hangs. Also cheaper: ask whether a **stand-in** already proves the property (`withBudget` against an unresumed continuation did), and be explicit about the gap it leaves rather than assuming a device closes it. |
| **🔴 A row cannot corroborate a claim about whether that row was written correctly.** The suspect artefact and the evidence are the same object, so it agrees with itself under both hypotheses. 2026-08-05, assessing pet-safety-eu#113: three orders under suspicion of being mis-recorded pickup orders all carried `shipping_cost=2490.00` — the home-delivery rate, against `1490`/`1990` on known pickup orders. That reads as strong corroboration and is worth **nothing**: `2490.00` is exactly what a genuine home order looks like *and* exactly what a pickup order mis-written as home looks like. A row written wrong is internally consistent about being wrong. | **Corroborate from an independent system, never from the artefact under suspicion.** Here the authority was the Stripe session's `metadata.delivery_method`, which is written by a different path and cannot inherit the row's defect. This is the generic-control failure (`grep -c 'the'`) wearing different clothes — it proves something was read, not that the thing you doubt is sound. **The danger is that it feels like diligence**, and it arrives pointing at the answer you already expect. Note when it is legitimate: a row *can* evidence a claim about something other than its own correctness. |

### `gshow` — the mitigation for the two rows above, mechanised

Both the `e3b0c442` row and the zsh row are **old**. They were in this table before 2026-08-02, and the trap still fired three times that day, once inside the commit that promoted them. **Knowledge was not the gap**, so a fourth row would not have helped. This is the version that cannot be skipped under time pressure — put it in your shell profile:

```bash
gshow() { git show "${1}:${2}" > /tmp/gshow.out || return 1
          printf '%s  %s bytes  %s lines\n' \
            "$(shasum -a 256 /tmp/gshow.out | cut -c1-12)" \
            "$(wc -c < /tmp/gshow.out)" "$(wc -l < /tmp/gshow.out)"; }
```

Two properties, and the second is the one that matters:

- **Braced**, so `${1}:${2}` cannot be eaten as a zsh parameter modifier.
- **`|| return 1` — a bad ref prints NOTHING and returns non-zero.** The actual failure mode all three times was *empty file, plausible hash, no error*. A wrong value invites a second look; a plausible one does not. This is the only response that cannot be misread in a hurry.
